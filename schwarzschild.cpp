...
tensor<2, double> Black_Hole::get_metric(double M, double x, double y,
                                         double z) {
  ...
  FOR2(i, j) {
    FOR2(k, l) { g[i][j] += g_spher[k][l] * jacobian[k][i] * jacobian[l][j]; }
  }
...
tensor<2, double> Black_Hole::calculate_spatial_inverse(tensor<2, double> g) {
...
Vec3 Black_Hole::eval_diff_eqn(double t, Vec3 v) {
  ...
  // Freezing out geodesics that are too close to Horizon (Metric is singular at
  // horizon)
  if (rr < 2 * M + eps) {
    FOR1(i) {
      ddx[i] = 0;
...
Vec3 Black_Hole::set_norm(Vec3 v, double norm_val, double M) {
